...
# Ex: ./tpm_mod_rs2.pl BLA_YPD_pvalues.tab BLA_MMS_pvalues.tab 0.001 > BLA_YPDvsMMS2_0.001.tsv
...
($ypd_names, $ypd_ids, $ypd_desc, $ypd_pvals) = &read_pvalues_2($ypd_file, $ID_hash);
($mms_names, $mms_ids, $ypd_desc, $mms_pvals) = &read_pvalues_2($mms_file, $ID_hash);

$com_ids = &get_overlap_set($ypd_ids, $mms_ids);
$n = scalar(@{$com_ids});
#printf STDERR "COMMON_ROW_IDS $n\n";
...
printf STDERR "TF\tYPD\tMMS\tBOTH\tR1\tR2\tCALL\n";
if(1) {

   $R_thr = 0.75;
   #printf STDERR "COMMON NAMES %s\n", join(" ", @{ $com_names });
   foreach $tf (sort @{ $com_names }) {
  	  
	  ($ypd_is, $mms_is, $bot_is) = &new_bound_in_both($tf, $ypd_pvals, $mms_pvals, $ypd_ids, $mms_ids, 
							 $t_liberal, $t_product, $t_strict);

     $n1 = $n2 = $n3 = 0;
	  if(@{ $ypd_is }){ $n1 = scalar(@{ $ypd_is }); }
	  if(@{ $bot_is }){ $n2 = scalar(@{ $bot_is }); }
	  if(@{ $mms_is }){ $n3 = scalar(@{ $mms_is }); }
		
	  @{ $new->{$tf} } = split(":", ("$neither:" x $n));
	
	  ## YPD    MMS   YPD^MMS
	  $R1 = $n2/($n1+$n2); ## BOTH/(YPD_ONLY + BOTH)
	  $R2 = $n2/($n2+$n3); ## BOTH/(MMS_ONLY + BOTH)
	  if(($R1>$R_thr) && ($R2>$R_thr)){ $call = "invarient"; }
	  elsif($R1>$R_thr){ $call = "expanded in MMS"; }
	  elsif($R2>$R_thr){ $call = "contracted in MMS"; }
	  else { $call = "shifted"; }
	  ...
	  @{ $new->{$tf} }[@{ $ypd_is }] = split(":", ("$ypd:" x $n1));
     @{ $new->{$tf} }[@{ $mms_is }] = split(":", ("$mms:" x $n3));
   }
}
